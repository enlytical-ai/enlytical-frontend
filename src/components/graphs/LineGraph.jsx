...
const LineGraph = (props) => {
    ...
    if (el) {
      if (startXAxisDateArray.length < 2 && endXAxisDateArray.length < 2) {
        ...
        NotificationManager.error(
          "You cannot select more than two areas.",
          "Warning",
          3000
        );
      }
  ...
  const onMouseUp = function (e) {
    ...
    if (el && mouseDownValue) {
      ...
      setEndXAxisDateArray((prevState) => [
        ...prevState,
        xAxisDataArray[index],
      ]);
    ...
    afterDatasetDraw(chart, args, pluginOptions) {
      ...
      for (let i = 0; i < pluginOptions.startDate.length; i++) {
        ...
        ctx.fillRect(
          ...
          top,
          x.getPixelForValue(endDate) - x.getPixelForValue(startDate),
          height
        );
      }
      ctx.rotate(90 * angle);
      for (let j = 0; j < pluginOptions.startDate.length; j++) {
        ...
        const diff =
          (x.getPixelForValue(endDate) - x.getPixelForValue(startDate)) / 2;
        ctx.translate(0, 0);
        ctx.fillStyle = "#666";
        ...
        ctx.fillText(
          `${startDate} - ${endDate}`,
          height / 2 + top,
          (x.getPixelForValue(startDate) + diff) * -1
        );
      }
      ctx.restore();
    },
