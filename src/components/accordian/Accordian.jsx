...
const Accordian = (props) => {
    ...
    useEffect(() => {
        ...
        if (accordianBodyHeight) rootEl.style.setProperty("--abysp_accordian_body_heigh", `${accordianBodyHeight - 1}px`);
        if (openingAanimationDuration) rootEl.style.setProperty("--abysp_opening_animation_duration", `${openingAanimationDuration}s`);
        if (closingAanimationDuration) rootEl.style.setProperty("--abysp_closing_animation_duration", `${closingAanimationDuration}s`);
        if (accordianBorderRadius) rootEl.style.setProperty("--abysp_accordian_border_radius", `${accordianBorderRadius}px`);
        if (accordianBorderColour) rootEl.style.setProperty("--abysp_accordian_border_colour", accordianBorderColour);
        if (accordianHeaderBackgroundColor) rootEl.style.setProperty("--abysp_accordian_header_background_color", accordianHeaderBackgroundColor);
        if (accordianHoverHeaderBackgroundColor) rootEl.style.setProperty("--abysp_accordian_hover_header_background_color", accordianHoverHeaderBackgroundColor);
        if (accordianBodyBackgroundColor) rootEl.style.setProperty("--abysp_accordian_body_background_color", accordianBodyBackgroundColor);
        if (accordianStatus) {
    ...
    return (
        <div className="abysp_accordian_ontainer" >
            <div id="abysp_accordian_header" style={{ height: accordianHeaderHeight ? (accordianHeaderHeight - 2) : 28 }} className={headerClass} >
