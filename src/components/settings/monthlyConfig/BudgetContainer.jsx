...
import {
  getMonthAndYearArray,
  getFirstDayOfMonthAndYearArray,
  getMontheInText,
} from "../../../utils/commonFunction";
import {
  divTwoNum,
  roundOffToTwoDecimal,
} from "../../../commonFunction/commomFunction";
import {
...
const BudgetContainer = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .post(
        `${BASE_URL}clientMonthlyConfig/monthlyBudget?brandId=${current_brand}`,
        {
      ...
      .then(function (response) {
        const {
          ...
          category_wise_sales_and_spend_target_business_as_usual,
          _id,
        } = response.data.data.clientMonthlyConfig;
        setState({
          _id,
          category_wise_sales_and_spend_target_deal_days,
          category_wise_sales_and_spend_target_business_as_usual,
  ...
  const onRowElSaveDealDays = (e) => {
    ...
    axios
      .put(
        `${BASE_URL}clientMonthlyConfig/${state._id}`,
        { category_wise_sales_and_spend_target_deal_days: arr },
        {
          headers: { token },
        }
      )
  ...
  const onRowElSaveBusinessAsUsual = (e) => {
    const token = localStorage.getItem("token");
    const arr = [
      ...state.category_wise_sales_and_spend_target_business_as_usual,
    ];
    const { category, ad_sales, spend } = e;
    arr.forEach((el) => {
      if (el.category === category) {
    ...
    axios
      .put(
        `${BASE_URL}clientMonthlyConfig/${state._id}`,
        { category_wise_sales_and_spend_target_business_as_usual: arr },
        {
          headers: { token },
        }
      )
  ...
  const brandConfigEditToggle = () => {
    ...
    axios
      .put(
        `${BASE_URL}brand?brandId=${current_brand}`,
        {},
        {
          headers: { token },
        }
  ...
  const dateComponent = (props) => {
    return <div> {getMandT(props.value)}</div>;
  };
          ...
          <div className="adSalesTargetInputContainer">
            <input
              ...
              onChange={(e) => {
                setState((state) => ({
                  ...state,
                  ad_sales: e.target.value * 1,
                }));
              }}
              className="adSalesTargetInput"
              value={state.ad_sales}
            ></input>
            <i onClick={() => onSaveFn()} class="bi bi-check-circle"></i>
          </div>
        ) : state.ad_sales === "" ? (
          addIconStatus ? (
            <i onClick={() => onDoubleClick()} class="bi bi-plus-circle"></i>
          ) : (
        ...
        {inputToggle ? (
          <div className="adSalesTargetInputContainer">
            <input
              ...
              type="number"
              onChange={(e) => {
                setState((state) => ({ ...state, spend: e.target.value * 1 }));
              }}
              className="adSalesTargetInput"
              value={state.spend}
            ></input>
    ...
    <div className="salesSpendContainer">
      <div className="salesSpendContainerHeader">
        ...
        <div className="salesSpendContainerDateContainer">
          ...
          >
            {searchDateArray &&
              searchDateArray.map((date, i) => {
                return (
                  <option
                    selected={searchDate === date ? true : false}
                    key={i}
                    value={date}
                  >
                    {" "}
